...
class ChipDistribution():
    ...
    def calcu(self, dateT, highT, lowT, avgT, volT, TurnoverRateT, minD=0.01, flag=1, AC=1.0):
        if flag == 1:
    ...
    def calcuChip(self, flag=1, AC=1.0):  # flag 使用哪个计算方式,    AC 衰减系数
        low = self.data['low']
        high = self.data['high']
        ...
        for i in range(len(date)):
            ...
            # print(date[i])
            dateT = date[i]
            self.calcu(dateT, highT, lowT, avgT, volT, TurnoverRateT / 100, flag=flag,
                       AC=AC)  # 东方财富的小数位要注意，兄弟萌。我不除100懵逼了

    # 计算winner 收盘获利
    def winner(self, close_price=None):
        Profit = []
        ...
        for i in self.ChipList:
            ...
            if step > 90:
                if bili < 0.05 and hold_status == 0:
                    hold_status = 1
                    print(date[step], bili, close_price[step])
                elif bili > 0.9 and hold_status == 1:
                    hold_status = 0
                    print(date[step], bili, close_price[step])
        ...
        # plt.plot(date[len(date) - 200:-1], Profit[len(date) - 200:-1], "o")
        # plt.show()
